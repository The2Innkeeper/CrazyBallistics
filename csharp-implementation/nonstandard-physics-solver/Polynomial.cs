...
public static class Polynomial
{
    ...
    /// </summary>
    /// <param name="coeffs">The coefficients of the polynomial in increasing order of degree, as a List<float>.</param>
    /// <returns>The LMQ bound as a float, representing an upper bound on the polynomial's positive roots.</returns>
    /// <exception cref="ArgumentException">Thrown when the coefficients list is null or empty.</exception>

    public static float LMQBound(List<float> coeffs)
    {
        // Validate input
        ...
        for (int neg_i = coeffs.Count - 1; neg_i >= 0; neg_i--)
        {
            if (!(coeffs[neg_i] < 0)) continue;
            ...
            for (int pos_i = coeffs.Count - 1; pos_i > neg_i; pos_i--)
            {
                if (!(coeffs[pos_i] > 0)) continue;

                // Compute radical
                double radical = Math.Pow(-Math.Pow(2, power) * coeffs[neg_i] / coeffs[pos_i], 1.0 / (pos_i - neg_i)); // $\sqrt[j-i]{\frac{-2^{t_j} a_i}{a_j}}$
