...
public class LMQLowerBoundTests
{
    ...
    [InlineData(new float[] { 1, 100, -100, -1 }, 1f / 2f)]
    // - 6x^3 + 11x^2 - 6x + 1
    // Smallest positive real root: 1/3 = 0.33333
    [InlineData(new float[] { 1, -6, 11, -6 }, 1f / 12f)]
    // x^4  - 4x^3 - x^2 + 2x + 3
    // As a reference, the smallest positive real root is x≈1.1137
    [InlineData(new float[] { 3, 2, -1, -4, 1 }, 1f / 1.74716093f)]
    public void LMQLowerBound_WithValidCoefficients_ReturnsCorrectBound(float[] coeffs, float expectedBound)
    {
        // Arrange
